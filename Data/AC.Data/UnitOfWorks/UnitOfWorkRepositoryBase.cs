...
namespace AC.Data
{
  ...
  public abstract class UnitOfWorkRepositoryBase : IUnitOfWorkRepository
  {
    ...
    private readonly bool _uowOwnsbuilderProvider = true;
    public bool IsConfigured { get; protected set; }

    private IRepositoryBuilderProvider _builderProvider;
    public IRepositoryBuilderProvider BuilderProvider
    {
      get
      {
        if (_builderProvider==null) throw new NullReferenceException("A repository provider is necessary!");
        return _builderProvider;
      }
      set
      {
    ...
    public UnitOfWorkRepositoryBase(bool uowOwnsbuilderProvider = true)
    {
      IsConfigured = false;
      Frozen = false;
      _builderProvider = new MemoryRepositoryBuilderProvider();
      Frozen = true;
      _uowOwnsbuilderProvider = uowOwnsbuilderProvider;
    }

    public UnitOfWorkRepositoryBase(IRepositoryBuilderProvider builderProvider, bool uowOwnsbuilderProvider = true)
    {
      IsConfigured = false;
      Frozen = false;
    ...
    {
      return BuilderProvider.GetRepository<TRepository>(this);
    }
